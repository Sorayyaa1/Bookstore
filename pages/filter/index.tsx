...
function Filter(){
    const {filterItems,setSelectedBook,setBtnValue}=useContext(BookContext)
    ...
    function handlerClick(event:React.MouseEvent<HTMLButtonElement>){
        ...
        router.push(`/filter/${buttonValue}`) 
        if(filterItems){
    ...
    return(
        <div>
            <div className="bg-amber-900 opacity-90">
                <h1 className="text-4xl text-amber-900 text-center py-8">Filter Book</h1>
            </div>
            <div>
                {
