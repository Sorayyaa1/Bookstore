...
function AllPoetryBooks(){
    ...
    const router=useRouter()
      ...
      function handleClick(event:React.MouseEvent<HTMLButtonElement>){
       ...
       router.push(`/category/SomeTitlesNon-Fiction-NarrativeBooksList/AllNonFictionNarrativeBooks/${buttonValue}`) 
   ...
   const PoetryBooksList=allBooksList.filter(item=>item.category==="non-Fiction-Narrative")


    return(
        <div className="bg-transparent">
           <div className="bg-amber-900 opacity-90">
                <h1 className="text-4xl text-amber-900 text-center py-8">All Non-Fiction-Narrative Books</h1>
            </div>
            <div className=" grid grid-cols-4 gap-8 p-10 bg-amber-600 opacity-75 ">
                {
                    PoetryBooksList.map((item,index)=>(
